...
interface AdminLingkunganPageProps {
...
export default function AdminLingkunganPage({ 
  currentUser, 
  onProfileClick, 
  onLogoutClick,
  adminActiveTab = "data",
  ...
  onSessionUpdate
}: AdminLingkunganPageProps) {
  const [alert, setAlert] = useState<{ type: "success" | "error"; message: string } | null>(null)

  const category = "Statistik Lingkungan Hidup & Multi-Domain"

  // Validation: ensure only admin_lingkungan can access this page
  if (currentUser && currentUser.role !== 'admin_lingkungan') {
    return (
      <div className="min-h-screen bg-background p-6">
        <div className="max-w-md mx-auto mt-20">
          <Alert className="border-red-200 bg-red-50">
            <AlertDescription className="text-red-800">
              Error: Halaman ini hanya untuk Admin Lingkungan. 
  ...
  // Temporary: Allow access for admin lingkungan - to be fixed later  
        ...
        <Card className="w-96 shadow-xl border-0">
          <CardHeader className="text-center">
            ...
            <CardDescription className="text-gray-600">
              Anda tidak memiliki akses ke halaman ini. Silakan login dengan akun admin lingkungan.
            </CardDescription>
          </CardHeader>
        </Card>
      </div>
        ...
        <div className="max-w-7xl mx-auto px-6 py-4">
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-2xl font-bold text-gray-900">Admin Lingkungan & Multi-Domain</h1>
                <p className="text-sm text-gray-600">
                  Kelola data statistik lingkungan hidup dan multi-domain Kabupaten Bungo
                </p>
              </div>
            </div>
            <div className="flex items-center gap-3">
              <Badge className="bg-orange-100 text-orange-800 px-3 py-1.5 font-medium border-0">
                {currentUser?.full_name || currentUser?.name || "Admin Lingkungan"}
              </Badge>
              <Button
                variant="outline"
                size="sm"
      ...
      <div className="max-w-7xl mx-auto px-6 py-6">
        ...
        <Tabs value={adminActiveTab} onValueChange={onAdminTabChange} className="space-y-6">
          ...
          <TabsContent value="import" className="space-y-6">
            <ExportDataComponent 
              category={category}
              title="Export Data Lingkungan & Multi-Domain"
